...
namespace AccessDB_Test
{
    ...
    class ExcelExe
    {
        ...
        {
            EXCEL_COM,         //COM API 사용(리소스 해제 확실히)
            OLEDB_EXCEL,       //OLEDB 사용(SQL쓰듯)
            CSV_STREAM,        //CSV 사용(, 로 Cell을 구분)
        }
        ...
        public string ExcelExport(string SaveFilePath, DataTable dt, enExportType ExportType)
        {
            ...
            switch (ExportType)
            {
                case enExportType.EXCEL_COM:
                    strRet = SaveExcel_COMAPI(SaveFilePath, dt);  // 호출한 메소드 작업 완료 후 성공,실패 여부 text return받아옴
                    break;
